...
$app->post("/login",function() use ($db, $app){
    ...
    $query = $db->query("SELECT * FROM `db_usuarios` where email = '".$userLogin."' and pass = '".$passLogin."' ");
    ...
    if(empty($row) == 0){
      ...
      echo json_encode( array("status"=>1 , "messege"=>"No existe usuarios para las credenciales ingresadas.") );
    }
...
$app->post("/create",function() use ($db, $app){
    ...
    if(!is_valid_email($app->request->post("email"))){
        echo json_encode(array("status"=>1,"messege"=>"El formato de email no es correcto."));
        exit;        
    }
...
$app->post("/mailing",function() use ($db, $app){
    ...
    if($row["cantidad"] > 0){
        echo json_encode(array("status"=>1,"messege"=>"El email ingresado ya está registrado."));
        exit;
    }
...
$app->post("/recovery",function() use ($db, $app){
    ...
    if(empty($email)){
        echo json_encode(array("status"=>1,"messege"=>"El atributo [email] es obligatorio y debe tener un valor. Revise el tipo de dato."));
        exit;
    }
    ...
    $row=$query->fetch_array();
    if($row["existe"]==0){
        echo json_encode(array("status"=>1,"messege"=>"El mail ingresado no figura en nuestros en nuestro sistema, por favor inténtelo de nuevo."));
        exit;
    }
    ...
    try{
        ...
        //Destinatario
        $mail->setFrom('mesadeayuda@mininterior.gob.ar', 'Genesis');
        $mail->addAddress($email);

        //Contenido
        $mail->isHTML(true);
        $mail->Subject = 'Recuperar el password.'; 
        ...
        $mail->Body .='                    <h3 style="text-align: center;"><b>Hola vamos a recuperar rapidamente tu acceso al sistema</b>. Solo necesitás hacer click <a href="'.$base_url.'module/template/recovery/'.$hash.'" target="_blank" style="color:#309bc3; text-decoration:none">en el siguiente enlace</a></h3>';
        $mail->Body .='                </td>';
        $mail->Body .='            </tr>';
        $mail->Body .='            <tr class="ui-draggable">';
        ...
        //Estados
        $messege = 'El mensaje se envio correctamente!';
        $status=0;

    }catch (Exception $e) {
        // $messege = 'Hubo un error al enviar el correo: '.$e;
        $messege = 'Hubo un error al enviar el correo, asegúrese que el correo pertenezca al ministerio.';
        $status=1;
    }
...
$app->put("/recovery/update",function() use ($db, $app){
    ...
    $error_encontrado="";
    if(!validar_clave($newpass,$error_encontrado)){
        echo json_encode(array("status"=>1,"messege"=>$error_encontrado));
        exit;        
    }
    ...
    $query = $db->query("UPDATE db_usuarios SET pass= '".$newpass."' WHERE hash_session = '".$hash_session."' ");
    if($query){
        $hash = md5(date("Y/m/d h:i:sa"));
...
$app->get("/usuario/(:id)", function ($id) use ($db, $app) {
 //si no existe el id
  ...
  //Si existe que me traiga los campos  
    ...
    $usuario = array(
        "id" => $id, "nombre" => $name, "email" => $mail, "owner" => $check
