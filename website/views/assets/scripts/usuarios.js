...
function altaUsuarios() {
  ...
  if ($("#nombreup").val() == "") {
    ...
    swal("Validación!", "El campo nombre es obligatorio", "warning");
    $("#statusEmail").html(
  ...
  if ($("#newemail").val() == "") {
    ...
    swal("Validación!", "El campo email/usuario es obligatorio", "warning");
    $("#statusEmail").html(
      '<span class="label label-error" style="color: red">Campo obligatorio</span>'
    );
    return false;
  ...
  if (newemail.indexOf(configmail) == -1) {
    swal(
      "Algo salió mal!",
      "El dominio de este mail [" +
        newemail.substr(newemail.indexOf("@")) +
        "] no corresponde al configurado en el sistema [Config.ini].",
      "error"
    );
    $("#statusEmail").html(
      '<span class="label label-error" style="color: red">Dominio de email no valido</span>'
    );
  ...
  if (pass1 != pass2) {
    swal("Validación!", "las contraseñas deben ser iguales", "warning");
    return false;
  }
  ...
  fetch(url_base + "api/perfiles/validateuser", {
    ...
    .then((respObj) => {
      ...
      if (respObj.status == 0) {
        ...
        $("#statusEmail").html(
          '<span class="label label-error" style="color: red">' +
            respObj.messege +
            "</span>"
        );
...
function checkValidPassword() {
  ...
  if (
    ...
    $("#statusSecondPassword").html(
      '<span class="label label-success" style="color: red">Deben ser iguales, tener entre 8 y 35 caracteres, contener letras, números y alguna letra en mayúscula.</span>'
    );
  } else {
...
function validarMail() {
  ...
  if ($("#newemail").val() == "") {
    ...
    $("#statusEmail").html(
      '<span class="label label-error" style="color: red">El formato no corresponde a un correo.</span>'
    );
    ...
    $("#statusEmail").html(
      '<span class="label label-error" style="color: red">El dominio de este mail no corresponde al configurado en el sistema.</span>'
    );
    inputemail.removeClass("is-valid");
    inputemail.addClass("is-invalid");
    inputemail.css("border-color", "red");
    ...
    $("#statusEmail").html(
      '<span class="label label-success" style="color: green">El email está correcto</span>'
    );
    inputemail.removeClass("is-invalid");
    inputemail.addClass("is-valid");
    inputemail.css("border-color", "green");
...
function escribirDatos() {
  ...
  var url_base = $("#url_base").val();
  //console.log("url:" + url_base + "api/perfiles/traerUsuarios");
  //gabi
  $.ajax({
      ...
      for (i = 0; i < datosArray.length; i++) {
        ...
        container.append(
          '<li class="list-group-item d-flex justify-content-between lh-sm">' +
            ...
            '<h6 class="nombreUsuario my-0 font-weight-bold">' +
            nombreusuario +
            "</h6>" +
            '<small class="my-0 text-info" id="userRange">' +
            userStatus +
            "</small>" +
            ...
            "','" +
            passusuario +
            '\');" data-toggle="modal" data-target="#updateUsuario"><i class="fas fa-edit fa-lg"></i></button>' +
            '<button type="button" class="btn btn-danger" onclick="deleteUsuario(' +
            id +
...
function validarMailUp() {
  ...
  if ($("#newemailUp").val() == "") {
    ...
    swal(
      "Algo salió mal!",
      "El dominio de este mail [" +
        newemailUp.substr(newemailUp.indexOf("@")) +
        "] no corresponde al configurado en el sistema [Config.ini].",
      "error"
    );
    $("#statusEmailUp").html(
...
function checkValidPasswordUp() {
  ...
  if (
    ...
    $("#statusSecondPasswordUp").html(
      '<span class="label label-success" style="color: red">Debe tener entre 8 y 35 caracteres, contener letras, números y alguna letra en mayúscula.</span>'
    );
  } else {
...
function deleteUsuario(id) {
  console.log("este es id del delte" + id);
  swal({
    title: "Estás seguro?",
    ...
    if (willDelete) {
      fetch('https://sistemas.mininterior.gob.ar/desagenesis/api/perfiles/deleteUsuario',{
        method: "DELETE",
        headers: new Headers({
          "Content-Type": "application/x-www-form-urlencoded",
        }),
    ...
    success: function (data) {
      ...
      var filtrar = () => {
        ...
        for (let dato of datosArray) {
          ...
          if (
            nombreusuario.indexOf(texto) != -1 ||
            emailusuario.indexOf(texto) != -1
          ) {
            container.innerHTML +=
              '<li class="list-group-item d-flex justify-content-between lh-sm">' +
              '<div id="infoUsuarios" class="d-flex flex-column">' +
              ...
              '<div class="widget-content">' +
              '<div role="group" class="btn-group" data-toggle="buttons">' +
              '<button type="button" class="btn btn-success" onclick="modificarUsuarioVertodos(' +
              id +
              ",'" +
              nombreusuario +
              "','" +
              ...
              "','" +
              passusuario +
              '\');" data-toggle="modal" data-target="#updateUsuarioVertodos"><i class="fas fa-edit fa-lg"></i></button>' +
              '<button type="button" class="btn btn-danger" onclick="deleteUsuario(' +
              id +
              ')"><i class="fas fa-trash fa-lg"></i></button>' +
              "</div>" +
...
function validarMailUpTodos() {
  ...
  if ($("#newemailUpTodos").val() == "") {
    ...
    $("#statusEmailUpTodos").html(
      '<span class="label label-error" style="color: red">Campo obligatorio</span>'
    );
    inputemailUpTodos.css("border-color", "red");
    return false;
  } else if (
    newemailUpTodos.indexOf("@", 0) == -1 ||
    newemailUpTodos.indexOf(".", 0) == -1
  ) {
    $("#statusEmailUpTodos").html(
      '<span class="label label-error" style="color: red">El formato no corresponde a un correo.</span>'
    ...
    newemailUpTodos.substr(newemailUpTodos.indexOf("@")) !=
    "@" + configmail
  ) {
    swal(
      "Algo salió mal!",
        ...
        for (let dato of datosArray) {
          ...
          if (
            ...
            container.innerHTML +=
              ...
              '<div class="widget-content">' +
              '<div role="group" class="btn-group" data-toggle="buttons">' +
              '<button type="button" class="btn btn-warning" id="fondoDeRecuperar" onclick="restablecerUsuario(' +
              id +
              ')"><i class="fas fa-trash-restore fa-lg" id="iconorecuperar"></i></button>' +
              "</div>" +
              "</div>" +
              "</li>";
          }
...
function restablecerUsuario(id) {
  ...
  swal({
    ...
    if (willDelete) {
      fetch('https://sistemas.mininterior.gob.ar/desagenesis/api/perfiles/recuperarUsuario',{
        method: "DELETE",
        headers: new Headers({
          "Content-Type": "application/x-www-form-urlencoded",
        }),
